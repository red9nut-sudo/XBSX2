...
static HANDLE GetSleepTimer() {
    if (s_sleep_timer_created) return s_sleep_timer;
    ...
    if (!s_sleep_timer) s_sleep_timer = CreateWaitableTimer(nullptr, TRUE, nullptr);
    return s_sleep_timer;
}

u64 GetTickFrequency() { return lfreq.QuadPart; }
...
std::string GetConsoleModelString() {
    ...
    if (SUCCEEDED(hr)) {
        switch (deviceInfo.deviceId) {
            case GAMING_DEVICE_DEVICE_ID_XBOX_ONE: model = "Xbox One"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_ONE_S: model = "Xbox One S"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_ONE_X: model = "Xbox One X"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_ONE_X_DEVKIT: model = "Xbox One X Developer Kit"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_SERIES_S: model = "Xbox Series S"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_SERIES_X: model = "Xbox Series X"; break;
            case GAMING_DEVICE_DEVICE_ID_XBOX_SERIES_X_DEVKIT: model = "Xbox Series X Developer Kit"; break;
            default: model = "Unknown Xbox model"; break;
        }
    } else {
...
bool Common::InhibitScreensaver(bool inhibit) {
    ...
    if (inhibit) flags |= ES_DISPLAY_REQUIRED;
    SetThreadExecutionState(flags);
    return true;
}

void Common::SetMousePosition(int x, int y) { SetCursorPos(x, y); }
...
bool Common::PlaySoundAsync(const char* filename) {
    ...
    if (!xaudio2) {
        if (FAILED(XAudio2Create(xaudio2.GetAddressOf(), 0, XAUDIO2_DEFAULT_PROCESSOR))) return false;
        if (FAILED(xaudio2->CreateMasteringVoice(&masteringVoice))) return false;
    }

    if (FAILED(MFStartup(MF_VERSION, MFSTARTUP_FULLCOMPAT))) return false;

    std::wstring wfilename(filename, filename + strlen(filename));
    ComPtr<IMFSourceReader> reader;
    if (FAILED(MFCreateSourceReaderFromURL(wfilename.c_str(), nullptr, &reader))) return false;

    ComPtr<IMFMediaType> nativeType;
    if (FAILED(reader->GetNativeMediaType(MF_SOURCE_READER_FIRST_AUDIO_STREAM, 0, &nativeType))) return false;
    ...
    while (true) {
        ...
        if (FAILED(reader->ReadSample(MF_SOURCE_READER_FIRST_AUDIO_STREAM, 0, &streamIndex, &flags, &timestamp, &sample)) ||
            (flags & MF_SOURCE_READERF_ENDOFSTREAM)) break;
        if (sample) {
...
void Threading::Sleep(int ms) { ::Sleep(ms); }
...
void Threading::SleepUntil(u64 ticks) {
    ...
    if (one_hundred_nanos_diff == 0) return;
    LARGE_INTEGER fti;
...
// UWP/Xbox stubs (no changes needed)
